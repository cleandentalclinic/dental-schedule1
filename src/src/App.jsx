...
function lsGet(key){ try{ const v=localStorage.getItem(key); return v?JSON.parse(v):null; }catch(_){ return null; } }
...
export default function App() {
  ...
  useEffect(()=>{
    const e=lsGet('mc2_employees'); if(e) setEmployees(e);
    const s=lsGet('mc2_schedule');  if(s) setSchedule(s);
    const c=lsGet('mc2_sunCnt');    if(c) setSundayCount(c);
    setLoaded(true);
  },[]);
  useEffect(()=>{ if(loaded) lsSet('mc2_employees',employees); },[employees,loaded]);
  useEffect(()=>{ if(loaded) lsSet('mc2_schedule', schedule);  },[schedule,loaded]);
  useEffect(()=>{ if(loaded) lsSet('mc2_sunCnt',   sundayCount); },[sundayCount,loaded]);
              ...
              <div style={{display:'flex',alignItems:'center',gap:'10px',flexWrap:'wrap'}}>
                <div style={{display:'flex',alignItems:'center',gap:'8px',background:'#FEF3C7',border:'1px solid #FDE68A',borderRadius:'8px',padding:'6px 12px'}}>
                  ...
                  <button onClick={()=>setSundayCount(c=>Math.max(1,c-1))}
                    style={{width:'22px',height:'22px',borderRadius:'50%',border:'1px solid #FDE68A',background:'#fff',cursor:'pointer',fontSize:'15px',color:'#92400E',fontWeight:'700',display:'flex',alignItems:'center',justifyContent:'center'}}>−</button>
                ...
                <button onClick={autoDistribute} style={S.btn('primary')}>✦ 자동 배분</button>
              </div>
            </div>
          ...
          <div>
            <div style={{...S.card,padding:'1.25rem',marginBottom:'1rem'}}>
              ...
              <div style={{marginBottom:'14px'}}>
                <label style={{fontSize:'11px',color:'#64748B',marginBottom:'6px',display:'block',fontWeight:'500'}}>희망 오프 요일</label>
              ...
              return (
                <>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(110px,1fr))',gap:'10px',marginBottom:'1rem'}}>
                    {[
                      {label:'평균 오프',value:`${avg('off')}일`,c:'#1D4ED8'},
                      {label:'평균 야간',value:`${avg('night')}회`,c:'#4338CA'},
                      {label:'평균 일요일',value:`${avg('sunWork')}회`,c:'#D97706'},
                      {label:'균등도',value:fairLabel,c:fairColor},
                    ].map(card=>(
                        ...
                        <div key={emp.id} style={{...S.card,padding:'1.25rem'}}>
                          ...
                          <div>
                            <div style={{display:'flex',justifyContent:'space-between',fontSize:'11px',color:'#94A3B8',marginBottom:'4px'}}>
                              <span>근무 비율</span><span style={{fontWeight:'600',color:'#475569'}}>{workPct}%</span>
                            </div>
                            <div style={{height:'6px',background:'#F1F5F9',borderRadius:'99px',overflow:'hidden'}}>
                              <div style={{width:`${workPct}%`,height:'100%',background:emp.color,borderRadius:'99px'}}></div>
                            </div>
                          </div>
                        </div>
                      );
